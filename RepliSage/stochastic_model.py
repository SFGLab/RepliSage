...
class StochasticSimulation:
    ...
    def run_stochastic_simulation(self, N_steps, MC_step, burnin, T, T_min, f=1.0, b=1.0, kappa=10.0, c_rep=None, c_ising1=0.0, c_ising2=0.0, c_comp=0.0, mode='Metropolis',rw=True):
        ...
        ising_norm1, ising_norm2, comp_norm = -c_ising1, -c_ising2, -c_comp
        ...
        self.Ms, self.Ns, self.Es, self.Es_ising, self.Fs, self.Bs, self.Rs, self.spin_traj, self.J = run_energy_minimization(
    ...
    def show_plots(self):
        ...
        Draw plots.
        '''
        make_timeplots(self.Es, self.Es_ising, self.Fs, self.Bs, self.Rs, self.burnin//self.MC_step, self.out_path)
...
def main():
    # Set parameters
    N_beads, N_lef = 2000, 200
    N_steps, MC_step, burnin, T, T_min, t_rep, rep_duration = int(1e5), int(4e2), int(1e3), 1.8, 0.0, int(2e4), int(4e4)
    f, b, kappa, c_rep = 1.0, 1.0, 1.0, 5.0
    c_state_field, c_state_interact, c_comp = 0.5, 2.0, 10.0
    mode, rw, random_spins = 'Metropolis', True, True
    
    # Define data and coordinates
    region, chrom =  [90293500, 106874700], 'chr14'
    ...
    sim.run_stochastic_simulation(N_steps, MC_step, burnin, T, T_min, f, b, kappa, c_rep, c_state_field, c_state_interact, c_comp, mode, rw)
    sim.show_plots()
    sim.run_openmm('CUDA')
